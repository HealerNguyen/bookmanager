	...
	<div>
        <v-layout row wrap>
			...
			<v-toolbar
				...
				<v-card-title class="title font-weight-regular">Sửa tác giả</v-card-title>
				<v-spacer></v-spacer>
			</v-toolbar>
	      </v-flex>
		</v-layout>
...
export default {
	...
	methods: {
		...
		getAuthorById() {
			...
			if (id) {
				axios.post('/api/authors/getByIds', {
					id: id
				}).then(response  => {
					console.log(response.data)
					this.name = response.data.author.name
					this.status = '' + response.data.author.status
					this.email = response.data.author.email
					this.date = response.data.author.birthday
					this.gender = '' + response.data.author.gender
